document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const taskHTML = `
                            <div class="task-item" data-task-id="${task.id}">
                                ...
                                <div class="task-actions">
                                    ...
                                    <i class="fas fa-check-circle confirm-task pepe" style="display: none;" title="Confirm Changes"></i>
                                    <i class="fas fa-times-circle discard-task pepe" style="display: none;" title="Discard Changes"></i>
                                </div>
                            </div>
    ...
    function enableEditMode(taskItem) {
        ...
        const taskDesc = taskItem.querySelector('.task-desc-short').innerText.slice(0, -3);
        const taskStatus = taskItem.querySelector('.task-status span').innerText.trim().toLowerCase().replace(' ', '_');

        taskItem.querySelector('.task-title').innerHTML = `<input type="text" class="edit-input edit-inputs" value="${taskTitle}" required>`;
        taskItem.querySelector('.task-desc-short').innerHTML = `<textarea class="edit-input edit-inputs" required>${taskDesc}</textarea>`;

        taskItem.querySelector('.task-status span').innerHTML = `
            <select class="edit-select edit-inputs">
    ...
    function discardChanges(taskItem) {
        ...
        taskItem.querySelector('.task-title').innerText = originalTitle;
        taskItem.querySelector('.task-desc-short').innerText = originalDesc.substring(0, 50) + '...';
        taskItem.querySelector('.task-status span').innerText = originalStatus.replace('_', ' ');
