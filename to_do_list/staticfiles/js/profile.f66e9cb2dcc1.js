document.addEventListener('DOMContentLoaded', function() {
    ...
    function enableEditMode(taskItem) {
        ...
        const taskDesc = taskItem.querySelector('.task-desc-short').innerText.slice(0, -3); // remove the '...'
        const taskStatus = taskItem.querySelector('.task-status span').innerText.trim().toLowerCase().replace(' ', '_');

        taskItem.querySelector('.task-title').innerHTML = `<input type="text" value="${taskTitle}" required>`;
        taskItem.querySelector('.task-desc-short').innerHTML = `<textarea required>${taskDesc}</textarea>`;
